...
Not long ago I bought a new laptop with a NVIDIA GPU. As part of a promotion I got gifted a copy of Pragmata (a game im loving so far).  
...
These are very simple steps where's the catch? The catch is that for this to work the NVIDIA app needs to recognize your GPU and it has to match the GPU of the pc you bought, and that's the problem, NVIDIA app works great on my laptop but for some reason it doesn't recognize the GPU.
...
I tried to create a bootable Windows USB using Rufus, but it didn't work. I could only manage to create an installation drive but not a **WindowsToGo USB**.
...
NVIDIA and most of big tech companies suck. Thankfully (and in big part thanks to Valve) we are closer and closer to linux being **_"mainstream"_** and having direct support from manufacturers.
